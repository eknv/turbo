    ...
    PeriodPicker.prototype.init = function () {
        ...
        if (that.options.clearButtonInButton) {
            that.button.find('.icon_clear').on('mousedown', function (e) {
                that.clear();
                if (that.options.onClearButtonClick && $.isFunction(that.options.onClearButtonClick)) {
                    if (that.options.onClearButtonClick.call(that) === false) {
                        return;
                    }
                }
                e.preventDefault();
    ...
    PeriodPicker.prototype.show = function (target) {
        ...
        if (!self.options.inline) {
            ...
            if (self.options.fullsize) {
                ...
                var element2Position = target != null ? target : $(self)[0].button;
                self.picker.css(self.getPosition(element2Position));
            }
        }
        this.regenerate();
