...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    // scroller
    var scrollerVirtualHeight;   // virtual height
    var realScrollableHeight;    // real scrollable height
    ...
    function init() {
      ...
      /**
       * re-use the same option object, this way the options can be changed on the fly
       * options = $.extend({}, defaults, options);
       */
      options = $.extend(options, defaults, $.extend({}, options));
      ...
      $container
          .empty()
          //kn@11216: the following line has been commented out so that the scroll bars are displayed
          // .css("overflow", "hidden")
          .css("outline", 0)
          .addClass(uid)
          .addClass("ui-widget");
    ...
    function getDataItem(activeRow) {
      if (data.getItem) {
        return data.getItem(activeRow);
      } else {
    ...
    function scrollTo(y) {
      y = Math.max(y, 0);
      y = Math.min(y, scrollerVirtualHeight - viewportH + (viewportHasHScroll ? scrollbarDimensions.height : 0));
    ...
    function updateRowCount() {
      ...
      var oldH = realScrollableHeight;
      scrollerVirtualHeight = Math.max(options.rowHeight * numberOfRows, viewportH - scrollbarDimensions.height);
      if (scrollerVirtualHeight < maxSupportedCssHeight) {
        // just one page
        realScrollableHeight = pageHeight = scrollerVirtualHeight;
        numberOfPages = 1;
        jumpinessCoefficient = 0;
      }  else {
        // break into pages
        realScrollableHeight = maxSupportedCssHeight;
        pageHeight = realScrollableHeight / 100;
        numberOfPages = Math.floor(scrollerVirtualHeight / pageHeight);
        jumpinessCoefficient = (scrollerVirtualHeight - realScrollableHeight) / (numberOfPages - 1);
      }

      if (realScrollableHeight !== oldH) {
        $canvas.css("height", realScrollableHeight);
        scrollTop = $viewport[0].scrollTop;
      }

      var oldScrollTopInRange = (scrollTop + offset <= scrollerVirtualHeight - viewportH);

      if (scrollerVirtualHeight == 0 || scrollTop == 0) {
        page = offset = 0;
        ...
        // scroll to bottom
        scrollTo(scrollerVirtualHeight - viewportH);
      }

      if (realScrollableHeight != oldH && options.autoHeight) {
        resizeCanvas();
      }
    ...
    function handleScroll() {
      ...
      if (vScrollDist) {
        ...
        if (vScrollDist < viewportH) {
          ...
          var oldOffset = offset;
          if (realScrollableHeight == viewportH) {
            page = 0;
          } else {
            page = Math.min(numberOfPages - 1, Math.floor(scrollTop * ((scrollerVirtualHeight - viewportH) / (realScrollableHeight - viewportH)) * (1 / pageHeight)));
          }
          offset = Math.round(page * jumpinessCoefficient);
          if (oldOffset != offset) {
            invalidateAllRows();
    ...
    function handleKeyDown(e) {
      ...
      if (!handled) {
        if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
          ...
          if (editable() && currentEditor && currentEditor.keyCaptureList) {
    ...
    function setActiveCellInternal(newCell, opt_editMode) {
      ...
      if (activeCellNode != null) {
        ...
        if (editable() && opt_editMode && isCellPotentiallyEditable(activeRow, activeCell)) {
    ...
    function makeActiveCellNormal() {
      ...
      if (activeCellNode) {
        var dataItem = getDataItem(activeRow);
        $(activeCellNode).removeClass("editable invalid");
        if (dataItem) {
          var column = columns[activeCell];
          var formatter = getFormatter(activeRow, column);
          activeCellNode.innerHTML = formatter(activeRow, activeCell, getDataItemValueForColumn(dataItem, column), column, dataItem, self);
    ...
    //todo/kn.. move this to utility class
    function isNull(value) {
      ...
      function editable() {
          ...
          if (typeof options.editable === "function") {
              gridEditable = options.editable();
          } else {
              gridEditable = options.editable;
          }
          return gridEditable
              && (isNull(options.stopEdit) || !options.stopEdit)
              && (isNull(currentEditor) || isNull(currentEditor.editable) || currentEditor.editable());
      }
    ...
    function makeActiveCellEditable(editor) {
      ...
      if (currentEditor.editable && !currentEditor.editable()) {
        makeActiveCellNormal();
        return;
      }
